...
class SpeechToTextNode(Node):
    def __init__(self): 
        ...
        # Dynamic vosk model path
        vosk_model_path = os.environ.get('VOSK_MODEL_PATH', 
                                       os.path.expanduser('~/rs_ws/src/ros-vlm/vosk-model-small-en-us-0.15'))
        if not os.path.exists(vosk_model_path):
            # Try alternative locations
            alt_paths = [
                # Relative to package source
                ...
                '/opt/ros/jazzy/share/ros_vlm/vosk-model-small-en-us-0.15'
            ]
            for path in alt_paths:
                ...
                raise FileNotFoundError(f"Vosk model not found. Tried: {vosk_model_path}, {alt_paths}")
        
        self.get_logger().info(f'Using vosk model at: {vosk_model_path}')
